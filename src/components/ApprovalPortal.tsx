...
import { PortalIdentity } from '../services/portal-auth';
import { ApprovalRecord, HandoverDraft, HandoverStage, SnagSeverity, WorkspaceState } from '../types';
...
export function ApprovalPortal({ workspace, identity, onUpdateHandover, onSignOut }: ApprovalPortalProps) {
  ...
  useEffect(() => {
    if (!workspace.sites.some((site) => site.id === adminSiteId)) setAdminSiteId(workspace.sites[0]?.id || '');
  }, [adminSiteId, workspace.sites]);
  ...
  const adminSiteOptions = workspace.sites.filter((site) => {
    const query = adminSiteSearch.trim().toLowerCase();
    return !query || [site.cowId, site.siteLabel, site.region, site.city].some((value) => value.toLowerCase().includes(query));
  }).slice(0, 20);
  ...
  function transition(handover: HandoverDraft, nextStage: HandoverStage, action: string, decision?: ApprovalRecord['decision'], comments = '') {
    if (!canRoleTransition(workspace.currentRole, handover.stage, nextStage)) {
  ...
  function renderAdmin() {
    const site = workspace.sites.find((item) => item.id === adminSiteId) || null;
    return <ScrollView contentContainerStyle={styles.content}><Text style={styles.pageEyebrow}>ADMINISTRATION</Text><Text style={styles.pageTitle}>Platform controls</Text><Text style={styles.pageHelper}>Configure workflow definitions, permissions and integrations for the authenticated workspace.</Text><View style={styles.adminSitePicker}><Text style={styles.panelEyebrow}>CHECKLIST PREVIEW</Text><Text style={styles.panelTitle}>Select a site master record</Text>{workspace.sites.length === 0 ? <Empty title="No site master data" body="Sync the CMDB before previewing site-specific checklist capabilities." /> : <><TextInput style={styles.filterInput} placeholder="Search COW ID, site, region or city" placeholderTextColor={COLORS.muted} value={adminSiteSearch} onChangeText={setAdminSiteSearch} /><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.adminSiteOptions}>{adminSiteOptions.map((option) => <Pressable key={option.id} style={[styles.adminSiteOption, option.id === adminSiteId && styles.adminSiteOptionActive]} onPress={() => setAdminSiteId(option.id)}><Text style={[styles.adminSiteOptionId, option.id === adminSiteId && styles.adminSiteOptionTextActive]}>{option.cowId || 'No COW ID'}</Text><Text style={[styles.adminSiteOptionName, option.id === adminSiteId && styles.adminSiteOptionTextActive]}>{option.siteLabel}</Text></Pressable>)}</ScrollView>{site && <Text style={styles.adminSelectedSite}>Selected: {site.cowId || 'No COW ID'} · {site.hasTruckHead ? 'Truck-head checklist enabled' : 'Standard checklist'}</Text>}</>}</View><View style={styles.adminGrid}><AdminCard eyebrow="USER ACCESS" title="Roles & regions" body="Supabase RLS policies scope region reviewers to their assigned region. Access follows the authenticated account role and region." status="Policy adapter ready" /><AdminCard eyebrow="CHECKLIST" title="Definitions & photo rules" body={site ? `${checklistForSite(site).length} inspection definitions are active. Truck-head logic is conditional per site master data.` : 'Select a valid site to preview its checklist. No site-specific rules are calculated without a site.'} status={site ? 'Configured' : 'Select a site'} /><AdminCard eyebrow="SHEET SYNC" title="Google Sheet mapping" body="Server-side sync endpoint and source_data preservation are ready for the approved sheet mapping." status="Credentials required" /><AdminCard eyebrow="STORAGE" title="Private evidence bucket" body="Evidence paths follow COW ID / HO ID / item / type / timestamp and are signed for review." status="Adapter ready" /></View><View style={styles.integrationNotice}><View style={styles.noticeIcon}><Text style={styles.noticeIconText}>i</Text></View><View style={styles.flexOne}><Text style={styles.noticeTitle}>Development environment</Text><Text style={styles.noticeText}>The portal uses the authenticated Supabase workspace. Site master data and plan values remain read-only here; workflow actions follow the signed-in role.</Text></View></View></ScrollView>;
  }
...
const styles = StyleSheet.create({
  ...
  accountMeta: { color: '#91A7BB', fontSize: 11, marginTop: 4 },
  signOutButton: { borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', borderRadius: 9, paddingHorizontal: 11, paddingVertical: 9, alignItems: 'center', marginTop: 14 },
  signOutText: { color: '#C8D7E3', fontSize: 11, fontWeight: '800' },
  ...
  headerSignOut: { borderWidth: 1, borderColor: COLORS.border, borderRadius: 9, paddingHorizontal: 10, paddingVertical: 9 },
  headerSignOutText: { color: COLORS.blue, fontSize: 10, fontWeight: '900' },
  ...
  adminSitePicker: { backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.border, borderRadius: 15, padding: 16, gap: 9 },
  adminSiteOptions: { gap: 8 },
  adminSiteOption: { minWidth: 130, maxWidth: 210, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.background, borderRadius: 10, padding: 10 },
  adminSiteOptionActive: { backgroundColor: COLORS.pale, borderColor: COLORS.blue },
  adminSiteOptionId: { color: COLORS.blue, fontSize: 10, fontWeight: '900' },
  adminSiteOptionName: { color: COLORS.ink, fontSize: 9, marginTop: 3 },
  adminSiteOptionTextActive: { color: COLORS.navy },
  adminSelectedSite: { color: COLORS.green, fontSize: 10, fontWeight: '900' },
